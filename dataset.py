...
def get_dlpfc_data(id, path=None):
    ...
    if (not path):
        path = os.path.join('spatial_dataset', 'DLPFC', section_id)
...
def get_human_breast_cancer_data(path=None):
    if (not path):
        path = 'spatial_dataset/human_breast_cancer/'
    adata = sc.read_visium(path=path)
    adata.var_names_make_unique()
...
def get_mouse_brain_cerebellum_data(path=None):
    if (not path):
        path = 'spatial_dataset/mouse_brain_cerebellum/'
...
def get_mouse_brain_sagittal_data(path=None, section=None, cut=None):
    ...
    if (not path):
        path = os.path.join('spatial_dataset', 'mouse_brain_sagittal', 'section_'+str(section), cut)
...
def get_mouse_kidney_coronal_data(path=None):
    if (not path):
        path = 'spatial_dataset/mouse_kidney_coronal/'
    adata = build_adata(spot_scale=1., 
                        root_path=path, 
...
def get_mouse_olfactory_bulb_ST_data(id, path=None):
    if (not path):
        path = 'spatial_dataset/mouse_olfactory_bulb_ST/'
...
def get_mouse_olfactory_bulb_Stereo_seq_data(path=None):
    if (not path):
        path = 'spatial_dataset/mouse_olfactory_bulb_Stereo-seq/'
...
def get_mouse_somatosensory_cortex_data(path=None):
    if (not path):
        path = 'spatial_dataset/mouse_somatosensory_cortex/'
    adata = build_adata(spot_scale=2., 
                        root_path=path, 
